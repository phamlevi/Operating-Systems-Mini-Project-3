...
int null(int p)
{
	...
	if(pid < 0)
	{
		printf(1, "Fork failed. TEST CASE 1 FAILED!\n");
		kill(getpid());
		exit();
	}
	else if(pid == 0)
...
int null2(int p)
{
	...
	if(pid < 0)
	{
		printf(1, "Fork failed. TEST CASE 2 FAILED!\n");
		exit();
	}
	else if(pid == 0)
	{
		...
		printf(1, "%x %x\n", badPtr, *badPtr);
		// this process should be killed. If not, then the test failed.
		printf(1, "TEST CASE 2 FAILED!\n");
		kill(p);
		exit();
	}
	else
		wait();
	printf(1, "TEST CASE 2 PASSED!\n");
	return 0;
}
